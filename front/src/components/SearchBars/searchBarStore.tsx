...
export default function SearchBarStore({ onSelect }: Props) {
  ...
  const filteredMaterials = allMaterialsStore?.filter((material) => {
    ...
    const matchText =
      ...
      material.category?.label.toLowerCase().includes(search); 
    return matchText ;
  }) || [];   
                ...
                {filteredMaterials.length > 0 ? (
                  filteredMaterials.map((material) => (
                    ...
                    <div className="text-sm text-gray-500">
                      {material.brand} – {material.model} - {material.reference} - {material.category?.label}
                    </div>
                  </li>
                  ))
                ) : (
